            ...
            .pipe(
                ...
                map((deletedElements: ReadonlyArray<T>): boolean => {
                    ...
                    for (const deletedElement of deletedElements) {
                        ...
                        if (currentIndex > -1) {
                            cache = cache.toSpliced(currentIndex, 1);
                            anyDeleted = true;
                        }
                    }
...
const onUpdate = <T>(
    ...
    return (
        ...
        // Make a copy of the list, to not alter the original list
        const cacheCopy: T[] = [...cache];

        const existingElementIndex: number = cacheCopy.findIndex((el: T): boolean =>
        ...
        if (filterNewOrUpdatedFn && !filterNewOrUpdatedFn(updated)) {
            if (existingElementIndex > -1) {
                ...
                cacheCopy.splice(existingElementIndex, 1);

                return [true, [...cacheCopy]];
            }

            // Just return
            return [false, [...cacheCopy]];
        }

        if (existingElementIndex > -1) {
            console.log(`Updating element: "${JSON.stringify(updated)}"`);
            cacheCopy[existingElementIndex] = updated;

            return [true, [...cacheCopy]];
        }

        // Add it to the list if not existing
        return [true, [...cacheCopy, updated]];
    };
};
