...
import type { IKnownPeer, INetworkState, TClientId } from '../../network/network.interfaces.js';
import type { NetworkHostInterface } from '../../network/network-host-interface.class.js';
import type { NetworkServer } from '../../network/abstract-network.server.js';
import type { NetworkClient } from '../../network/abstract-network.client.js';
import {
    Persistica,
...
export class Network {
        ...
        this.networkStore
            ...
            .subscribe({
                next: (networkState: INetworkState | undefined) => {
                    if (networkState) {
                        this.networkState = structuredClone(networkState);
                    }
                },
                error: () => {
    ...
    public joinRequest(
        ...
        return structuredClone(this.networkState);
    };
    ...
    private clientConnected(
        ...
        const client: IKnownPeer | undefined = this.networkState.knownPeers.find((peer: IKnownPeer) => peer.clientId === clientId);
        if (client) {
