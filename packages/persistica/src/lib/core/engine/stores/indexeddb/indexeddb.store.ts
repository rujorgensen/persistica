...
type TTableNamettt<T> = keyof T extends string ? keyof T : never;

// ******************************************************************************
// *** Implementation
// ******************************************************************************
export class IndexedDBStore<TableTypeMap> implements PersistenceWrapper {
    ...
    constructor(
        ...
        private readonly tableTypeParser: TDataParsers<TTableNamettt<TableTypeMap>>,
    ...
    // * CRUD
    public async create<ReturnType>(tableName: TTableNamettt<TableTypeMap>, key: ReturnType, dontEmit?: boolean): Promise<ReturnType>;
    public async create<ReturnType>(tableName: TTableNamettt<TableTypeMap>, key: ReadonlyArray<ReturnType>, dontEmit?: boolean): Promise<ReadonlyArray<ReturnType>>;
    public async create<ReturnType>(
        tableName: TTableNamettt<TableTypeMap>,
     ...
     */
    public read<TableName_ extends string & keyof TDataParsers<TTableNamettt<TableTypeMap>>, ReturnType>(
    ...
    public async readMany<TableName_ extends string & keyof TDataParsers<TTableNamettt<TableTypeMap>>, ReturnType>(
        tableName: TableName_,
    ...
    public async update<ReturnType>(tableName: TTableNamettt<TableTypeMap>, key: ReturnType, dontEmit?: boolean): Promise<ReturnType>;
    public async update<ReturnType>(tableName: TTableNamettt<TableTypeMap>, key: ReadonlyArray<ReturnType>, dontEmit?: boolean): Promise<ReadonlyArray<ReturnType>>;
     ...
     * 
     * @param { TableName }                                 tableName
     * @param { TUniqueIdentifier | TUniqueIdentifier[] }   keyOrKeys
     *  
     * @returns { Promise<void> } 
     */
    public async delete<ReturnType>(tableName: TTableNamettt<TableTypeMap>, key: TUniqueIdentifier, dontEmit?: boolean): Promise<ReturnType>;
    public async delete<ReturnType>(tableName: TTableNamettt<TableTypeMap>, key: TUniqueIdentifier[], dontEmit?: boolean): Promise<ReadonlyArray<ReturnType>>;
    public async delete<ReturnType>(
        tableName: TTableNamettt<TableTypeMap>,
        keyOrKeys: TUniqueIdentifier | TUniqueIdentifier[],
        dontEmit?: boolean, // ! TODO 
    ): Promise<ReturnType | undefined | ReadonlyArray<ReturnType>> {
        const idxName: string | undefined = this.getUniqueIdentitier[tableName].indexName;
        const db = await this.initOrOpenIDB();

        if (Array.isArray(keyOrKeys)) {
            const toDelete: ReturnType[] = await this.getParsedItemsByKey(tableName, keyOrKeys);
        ...
        const toDelete: ReturnType | undefined = await this.getParsedItem(tableName, keyOrKeys, idxName);
        ...
        return new Promise((resolve, reject): void => {
            ...
            if (idxName) {
                // Find the real key using the custom index
                const index = store.index(idxName);

                // First, get the primary key associated with the uid
                const getKeyRequest = index.getKey(keyOrKeys);

                getKeyRequest.onsuccess = () => {
                    const primaryKey = getKeyRequest.result;
                    if (primaryKey !== undefined) {
                        const deleteRequest = store.delete(primaryKey); // Delete by primary key
                        console.log(`Deleting entry with primary key: ${primaryKey}`);
            ...
            const deleteRequest = store.delete(keyOrKeys);
     ...
     */
    private async getParsedItem<TableName_ extends string & keyof TDataParsers<TTableNamettt<TableTypeMap>>, ReturnType>(
        tableName: TableName_,
    ...
    private async getParsedItemsByKey<ReturnType>(
        ...
        return new Promise((resolve, reject): void => {
            ...
            request.onsuccess = (event: Event) => {
                const result: TDateAsString<ReturnType>[] = (event.target as IDBRequest).result;
                console.log(`⚡ ✅ Database key '${uids}' read`);
