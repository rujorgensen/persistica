...
const DATABASE_VERSION = 4 as const;
...
export class IndexedDBNetworkStore {
    ...
    public onUpdate(
        ...
        return this.eventSource.onUpdate$$
            .pipe(
                filter((networkState: INetworkState | (readonly INetworkState[])) => {
                    if (Array.isArray(networkState)) {
                        throw new Error('Expected single element');
                    }

                    return (networkState as INetworkState).networkId === networkId;
    ...
    private async getParsedItem(
        ...
        return new Promise((resolve, reject): void => {
            ...
            const request = store.index('networkId').get(networkId);
